...
module wave_ridges() {
  ...
  for (i = [0 : 60]) {
    ...
    if (pos < L) {
      ...
      // Generate ribs everywhere along the face. The screen_window_cut()
      // subtraction in front_shell() will clip the rib portions that cross
      // the screen window, leaving them surrounding the window cleanly.
...
module mount_bosses() {
  ...
  for (u = [u_left, u_right])
    for (v = [v_top, v_bot])
      on_front_face(v, u) {
        // Local +z = OUTWARD face normal, so -z goes INTO the cavity.
        // Boss starts at inner wall surface (z=-wall) and extends boss_h
        // further into the cavity. PCB rests on the boss tip.
        difference() {
          translate([0, 0, -wall - boss_h])
            cylinder(d=6, h=boss_h, $fn=24);
          // Pilot hole for M2.5 heat-set insert (Ø3.5 × 4.5 deep from tip)
          translate([0, 0, -wall - boss_h - 0.1])
            cylinder(d=3.5, h=4.6, $fn=20);
        }
      }
}
...
if (PART == "front_shell") {
  ...
  front_shell();
  translate([W_front + 30, 0, 0]) back_cover();
}
