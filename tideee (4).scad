...
// All rib geometry: every rib is a wavy_wrap_ring spanning the full face height.
// The side-wall reach tapers linearly from almost nothing at the top to the full
// back depth at the bottom, forming one unbroken diagonal on each side wall.
// screen_window_cut() in front_shell() carves the screen window opening.
module wave_ridges() {
  L          = face_len - ridge_y_start - 4;
  n_rows_est = ceil(log(1 + L * (ridge_ratio - 1) / ridge_gap_base) / log(ridge_ratio));
  y_shallow  = 5;           // side-depth (mm) of the topmost ring — barely clips the corner
  y_deep     = D - r_back;  // side-depth (mm) of the bottommost ring (43 mm)
  v_max      = face_len - ridge_y_start;  // ~118 mm — full face span for taper
  ...
  for (i = [0 : 60]) {
    ...
    if (pos < L) {
      ...
      tw      = v / v_max;   // 0 at face top → 1 at face bottom
